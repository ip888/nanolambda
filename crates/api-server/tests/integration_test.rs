...
async fn test_create_python_function() {
    ...
    let config = nanolambda_storage::FunctionConfig {
        ...
        code: "def handler(event, context):\n    return {'result': event.get('x', 0) + 1}".to_string(),
        memory_mb: 128,
        timeout_ms: 30000,
...
async fn test_update_function() {
    ...
    server.storage().update_function("update-test", updated_config).unwrap();
    
    // Verify update
    let function = server.storage().get_function("update-test").unwrap().unwrap();
...
async fn test_delete_function() {
    ...
    assert!(function.is_none() || function.unwrap().status == nanolambda_storage::FunctionStatus::Deleted);
