//! Request handlers for the NanoLambda API server.
//!
//! This module implements all HTTP request handlers for the serverless platform,
//! integrated with the storage layer for persistence and the runtime layer for
//! function execution.
//!
//! # Architecture
//!
...
//! 2. Perform authorization checks (API key validation)
//! 3. Execute business logic via storage/runtime managers
...
//! - **Free**: 1,000 invocations/month
//! - **Pro**: 100,000 invocations/month
//! - **Scale**: 1,000,000 invocations/month
//! - **Enterprise**: Unlimited
//!
//! # Error Handling
//!
//! All handlers return consistent error responses with:
//! - Appropriate HTTP status codes
//! - JSON body with `error` field containing a human-readable message
...
pub async fn invoke_function(
    ...
    match execution_result {
        Ok(exec_result) => {
            ...
            // Record invocation with proper error handling and logging
            match state.storage().record_invocation(invocation_record) {
                Ok(_) => {
                    tracing::debug!(
                        request_id = %request_id,
                        function_name = %name,
                        execution_time_ms = exec_result.metrics.execution_ms,
                        "Successfully recorded invocation"
                    );
                }
                Err(e) => {
                    tracing::warn!(
                        ...
                        request_id = %request_id,
                        function_name = %name,
                        "Failed to record invocation to storage, but execution succeeded"
                    );
                    // Record storage failure as a separate metric for monitoring
                    let error_metric = crate::metrics::MetricPoint {
                        timestamp: completed_at,
                        function_name: format!("{}_storage_error", name),
                        cold_start: false,
                        execution_time_ms: 0,
                        status: crate::metrics::MetricStatus::Error,
                    };
                    state.metrics().record(error_metric).await;
                }
            }
...
pub async fn health_check(
    ...
    let db_status = match state.storage().list_functions() {
...
mod tests {
    ...
    async fn test_health_check() {
        ...
        assert_eq!(response.0.database.as_deref(), Some("connected"));
    }
