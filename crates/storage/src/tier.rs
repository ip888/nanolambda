...
impl FromStr for TierLevel {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s.to_lowercase().as_str() {
            "starter" => Ok(TierLevel::Starter),
            "pro" => Ok(TierLevel::Pro),
            "enterprise" => Ok(TierLevel::Enterprise),
...
impl TierManager {
    ...
    pub async fn get_user_tier(&self, api_key: &str) -> Result<UserTier, sqlx::Error> {
        ...
        let tier = TierLevel::from_str(&tier_str)
            .ok()
    ...
    pub async fn get_upgrade_recommendation(
        &self,
        api_key: &str,
    ) -> Result<Option<UpgradeRecommendation>, sqlx::Error> {
        let user_tier = self.get_user_tier(api_key).await?;
        ...
        if invocation_increase > 0 {
            reasons.push(format!(
                "Get {}x more invocations",
                invocation_increase / 100 + 1
            ));
        }

        let memory_increase =
            recommended_config.max_memory_mb as i32 - current_config.max_memory_mb as i32;
...
mod tests {
    ...
    async fn test_tier_assignment() {
        ...
        let user_tier = manager
            .assign_tier("test_key_1", TierLevel::Pro)
            .await
            .unwrap();
    ...
    async fn test_monthly_counter() {
        ...
        manager
            .assign_tier("test_key_2", TierLevel::Starter)
            .await
            .unwrap();

        for _ in 0..5 {
            manager
                .increment_monthly_invocations("test_key_2")
                .await
                .unwrap();
        }
