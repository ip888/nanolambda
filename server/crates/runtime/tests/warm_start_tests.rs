...
"#;
    ...
    for i in 0..10 {
        ...
        println!(
            "Warm iteration {}: {}ms (cold_start: {})",
            i,
            duration.as_millis(),
            result.metrics.is_cold_start
        );
    }
    ...
    assert_eq!(
        warm_cold_count, 1,
        "Should have exactly 1 cold start in warm mode"
    );
    ...
    if warm_avg > 0 && cold_avg > 0 {
        ...
        assert!(
            speedup >= 2.0,
            "Warm starts should be at least 2x faster (actual: {:.2}x)",
            speedup
        );
    }
}
...
"#;
    ...
    for i in 1..=5 {
        ...
        if let Some(result_str) = result.result {
            ...
            assert!(
                result_str.contains(&expected_sum.to_string()),
                "Result should contain correct sum"
            );
            assert!(
                result_str.contains(&expected_product.to_string()),
        ...
        if i > 1 {
            assert!(
                !result.metrics.is_cold_start,
                "Iteration {} should be warm start",
                i
            );
        }
    }
...
"#;
    ...
    for i in 0..3 {
        ...
        let result1 = executor
            .execute(config1.clone(), event.clone())
            .expect("Func1 execution failed");
        let result2 = executor
            .execute(config2.clone(), event.clone())
