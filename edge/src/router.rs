...
async fn route_request(
    ...
    match (method.clone(), segments.as_slice()) {
        // Functions CRUD
        (Method::Get, ["functions"]) => handlers::functions::list_functions(env, &auth_ctx).await,
        ...
        (Method::Post, ["vectors", "query"]) => handlers::vectors::query(req, env, &auth_ctx).await,
        (Method::Post, ["vectors", "delete"]) => {
            handlers::vectors::delete(req, env, &auth_ctx).await
        }
        (Method::Get, ["vectors", "stats"]) => handlers::vectors::stats(env, &auth_ctx).await,
        (Method::Post, ["vectors", "namespaces"]) => {
            handlers::vectors::list_namespaces(env, &auth_ctx).await
        }

        // AI endpoints
        (Method::Post, ["ai", "embeddings"]) => handlers::ai::embeddings(req, env, &auth_ctx).await,
        (Method::Post, ["ai", "completions"]) => {
            handlers::ai::completions(req, env, &auth_ctx).await
        }
        (Method::Post, ["ai", "chat"]) => handlers::ai::chat(req, env, &auth_ctx).await,
        ...
        (Method::Delete, ["cache"]) => handlers::cache::clear_cache(env, &auth_ctx, None).await,
