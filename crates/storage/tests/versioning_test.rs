...
use nanolambda_storage::{FunctionConfig, StorageManager};
...
fn test_function_versioning_workflow() {
    ...
    let func_v1_specific = storage
        .get_function_by_version("test-func", 1)
    ...
    let versions = storage
        .list_function_versions("test-func")
    ...
    let func_v3_id = storage
        .publish_version("test-func", config_v3)
        .expect("Failed to publish v3");
...
fn test_version_isolation() {
    ...
    let id_v2 = storage
        .publish_version("math-func", config_v2)
        .expect("Failed to publish v2");
    ...
    let v1 = storage
        .get_function_by_version("math-func", 1)
        .expect("Failed to get v1")
        .expect("v1 not found");
    let v2 = storage
        .get_function_by_version("math-func", 2)
    ...
    assert_ne!(
        v1.code_hash, v2.code_hash,
        "Code hashes should be different"
    );
