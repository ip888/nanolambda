...
"#, function_code, handler_name);
        ...
        let stdin = child.stdin.take().ok_or_else(|| {
            PoolError::SpawnFailed("Failed to capture stdin".to_string())
        })?;

        let stdout = child.stdout.take().ok_or_else(|| {
            PoolError::SpawnFailed("Failed to capture stdout".to_string())
        })?;
    ...
    fn update_metrics(&mut self) -> Result<()> {
        ...
        match ProcessMetrics::from_pid(pid) {
            ...
            Err(e) => {
                Err(PoolError::CommunicationFailed(format!(
    ...
    fn get_peak_memory_mb(&self) -> u64 {
        self.metrics
            .as_ref()
            .map(|m| (m.rss_peak_bytes / 1024 / 1024) as u64)
            .unwrap_or(0)
    }
...
impl ProcessPool {
    ...
    /// Returns: (success, result, error, execution_ms, is_cold_start, memory_mb, peak_memory_mb, cpu_percent)
    pub fn execute_warm(
        &self,
        function_id: i64,
        version: i64,
        ...
        let mut processes = self.processes.lock().unwrap();
        let process = processes.get_mut(&cache_key)
            .ok_or(PoolError::CommunicationFailed("Process disappeared".to_string()))?;
        ...
        Ok((success, result, error, execution_ms, is_cold_start, memory_mb, peak_memory_mb, cpu_percent))
    }
...
"#;
        ...
        assert!(success3);
        assert!(is_cold3); // Different version = cold start
    }
}
